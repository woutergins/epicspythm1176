...
class thm1176():
    ...
    def __init__(self, **kwargs):
        ...
        self.api = kwargs.pop('api', 'visa')
        self.time_offset = time.time()
        r = kwargs.pop('address', None)
        if r is None:
            raise ValueError('address/device not given')

        if self.api.lower() == 'visa':
            self.device = visa.ResourceManager().open_resource(r)
            self.device.read_termination = '\n'

            self.device.chunk_size = 49216  # (4096 samples * 3 axes * 4B/sample + 64B for time&temp&...
            self.device.timeout = 10000
        elif self.api.lower() == 'usbtmc':
            self.device = usbdevice(r)
            self.device.device.reset()
            self.device.max_transfer_size = 49216
            self.device.timeout = 10
        else:
            raise ValueError("Unknown API '{}', use either 'visa' or 'usbtmc'".format(self.api))
        def cleanup():
